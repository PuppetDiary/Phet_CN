...
import { playgroundFeedbackRules } from './PlaygroundFeedbackRules.js';
...
export default class EnergySkateParkPlaygroundScreenView extends EnergySkateParkScreenView {
  ...
  public constructor(model: EnergySkateParkPlaygroundModel, tandem: Tandem, options?: EnergySkateParkScreenViewOptions) {
    ...
    playgroundFeedbackRules.forEach(rule => this.feedbackManager.registerRule(rule));
    ...
    this.tutorialOverlay = new TutorialOverlayNode({
      ...
      steps: [
        {
          // Step 1: Full-screen welcome (no cutout)
          target: null,
          title: '自由搭建轨道',
          content: '这里是游乐场模式！你可以自由搭建轨道，\n拖拽不同的轨道片段到画面上，\n拼接出你想要的轨道形状。',
          position: 'center'
        },
        {
          // Step 2: Track toolbox — stable, bottom-center node
          target: this.trackToolbox,
          title: '轨道工具箱',
          content: '从这里拖拽轨道片段放到画面上，\n可以自由拼接出各种轨道形状。\n试着搭建一条大弧线或环形轨道！',
          position: 'top'
        },
        {
          // Step 3: Clear (eraser) button — stable, small button
          target: this.clearButton,
          title: '清除轨道',
          content: '点击橡皮擦按钮可以清除所有轨道，\n重新开始搭建。\n试着设计出让滑板飞得最高的轨道！',
          position: 'top'
        },
        {
          // Step 4: Control panel — gravity combo box, mass slider
          target: this.controlPanel,
          title: '尝试不同环境',
          content: '在控制面板中切换重力环境（月球、木星……），\n看看同一条轨道在不同重力下，\n滑板的表现会有什么不同！',
          position: 'left'
        }
      ],
      onComplete: () => {
